...
`InstallHooks` deliberately names what it does: it validates the uploaded SHA-256 and invokes the extracted KPM install hook. It is not a claim that KPM itself installed the package. A true KPM clean-install/upgrade/uninstall test remains a separate release gate; that gate was exercised for Ink Chess 2.0.0 on the KT6 target on 2026-08-31, with evidence retained by the local hardware lab.
